...
__global__ void cuda_sgemm_v6(float *A_ptr, float *B_ptr, float *C_ptr, int M, int N, int K)
{
    ...
    int load_a_gmem_m = yBase + load_a_smem_m;
    int load_b_gmem_n = xBase + load_b_smem_n;
    ...
    {  
        ...
        int load_a_gmem_addr = OFFSET(load_a_gmem_m, load_a_gmem_k, K);
        FETCH_FLOAT4(r_load_a[0]) = FETCH_FLOAT4(A_ptr[load_a_gmem_addr]);
        int load_b_gmem_k = kBase + load_b_smem_k;
        int load_b_gmem_addr = OFFSET(load_b_gmem_k , load_b_gmem_n , N);
        FETCH_FLOAT4(r_load_b[0]) = FETCH_FLOAT4(B_ptr[load_b_gmem_addr]);
        ...
        a_shared[smem_sel_next][load_a_smem_k + 3][load_a_smem_m] = r_load_a[3];
        FETCH_FLOAT4(b_shared[smem_sel_next][load_b_smem_k][load_b_smem_n]) = FETCH_FLOAT4(r_load_b[0]);
...
int main()
{
    ...
    cuda_sgemm_v6<N_TILE_SIZE, M_TILE_SIZE, K_TILE_SIZE, N_PER_THREAD, M_PER_THREAD><<<grid, block>>>(matrix_A_device, matrix_B_device, matrix_C_device, M, N, K);
